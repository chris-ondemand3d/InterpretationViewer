...
class MPRWindow(QObject):
    ...
    def initialize(self):
        ...
        coronal_item = self._win.rootObject().findChild(QObject, 'vtk_coronal')
        sagittal_item = self._win.rootObject().findChild(QObject, 'vtk_sagittal')
        axial_item = self._win.rootObject().findChild(QObject, 'vtk_axial')
        ...
        for k, v in self.items.items():
            ...
            if _slice:
                ...
                _item.setHeight(_w)
                _item.setWidth(_h)
                _item.installEventFilter(self._win) # to grab mouse hover leave, add eventfilter
